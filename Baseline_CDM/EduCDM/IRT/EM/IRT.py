...
class IRT(CDM):
    ...
    def eval(self, test_data) -> tuple:
        ...
        for i in tqdm(test_data, "evaluating"):
            ...
            y_pred.append(pred_score[stu, test_id])
        ...
        return roc_auc_score(y_true, y_pred), np.sqrt(np.mean((np.array(y_true) - np.array(y_pred)) ** 2)) , accuracy_score(y_true, np.array(y_pred) >= 0.5)
