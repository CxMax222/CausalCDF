...
class Net(nn.Module):
    ...
    def forward(self, stu_id, input_exercise, input_knowledge_point, diff):
        ...
        scores_with_diff = torch.mul(output_1, diff.unsqueeze(1))
...
class KaNCD(CDM):
    ...
    def train(self, train_set, valid_set, lr=0.002, device='cpu', epoch_n=15):
        ...
        for epoch_i in range(epoch_n):
            ...
            for batch_data in tqdm(train_set, "Epoch %s" % epoch_i):
                ...
                pred = self.net(user_info, item_info, knowledge_emb,diff)
            ...
            accuracy, precision, rmse, recall, auc, f1 = self.eval(valid_set, device)
    ...
    def eval(self, test_data, device="cpu"):
        ...
        for batch_data in tqdm(test_data, "Evaluating"):
            ...
            knowledge_emb: torch.Tensor = knowledge_emb.to(device)
            diff: torch.Tensor = diff.to(device)
            pred = self.net(user_id, item_id, knowledge_emb,diff)
